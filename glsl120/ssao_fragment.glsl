...
void main()
{
    ...
    vec3 fragPos = texture2D(position_map, texCoords).xyz;
    float depth = texture2D(position_map,texCoords).w;
    if(depth>0.99999)
    {
        vec3 normal = texture2D(normal_map, texCoords).rgb;
        vec3 randomVec = texture2D(noise_map, texCoords * noise_scale).xyz;
        ...
        for(int i = 0; i < kernel_size; ++i)
        {
            ...
            // get sample depth
            float sampleDepth = -texture2D(position_map, offset.xy).w; // Get depth value of kernel sample
