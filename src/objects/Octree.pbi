...
DeclareModule Octree
  ...
  Structure Octree_t Extends Node_t
    *root.Node_t
  ...
  Declare New(*root.Node_t)
  Declare Delete(*o.Octree_t)
  Declare.b GetNode( *o.Octree_t, x.i, y.i , z.i, e0.i)
  Declare.b GetNodeFromPos(*o.Octree_t, x.f, y.f, z.f, e0.i)
  Declare.b Increment(*o.Octree_t)
  Declare LastNode(*o.Octree_t, x.i, y.i, z.i)
  Declare FirstLastNode(*o.Octree_t)
  Declare.b NextLastNode(*o.Octree_t)
EndDeclareModule
...
Module Octree
  ...
  ; CONSTRUCTOR (OCTREE)
  ;---------------------------------------------------------------------
  Procedure New(*root.Node_t)
    Protected *o.Octree_t = AllocateMemory(SizeOf(Octree_t))
    InitializeStructure(*o, Octree_t)
    *o\root = *root
    ProcedureReturn *o
  EndProcedure
  ...
  ; GET NODE
  ;---------------------------------------------------------------------
  Procedure.b GetNode(*o.Octree_t, x.i, y.i, z.i, e0.i)
    x = x & #OCTREE_MASK
    y = y & #OCTREE_MASK
    z = z & #OCTREE_MASK
  ...
  ;---------------------------------------------------------------------
  Procedure.b GetNodeFromPos(*o.Octree_t, x.f, y.f, z.f, e0.i)
  ...
  ; INCREMENT SEARCH
  ;---------------------------------------------------------------------
  Procedure.b Increment(*o.Octree_t)
  ...
  ; LAST NODE
  ;---------------------------------------------------------------------
  Procedure LastNode(*o.Octree_t, x.i, y.i, z.i)
    x & #OCTREE_MASK
    y & #OCTREE_MASK
    z & #OCTREE_MASK
    Protected difference.i = Int(Pow(*o\posxyz[0], x)) | Int(Pow(*o\posxyz[1], y)) | Int(Pow(*o\posxyz[2], z))
  ...
  ; FIRST LAST NODE
  ;---------------------------------------------------------------------
  Procedure FirstLastNode(*o.Octree_t)
    *o\e=#MAX_EXPONANT
    *o\posxyz[0]=0
    *o\posxyz[1]=0
    *o\posxyz[2]=0
  ...
  ; NEXT LAST NODE
  ;---------------------------------------------------------------------
  Procedure.b NextLastNode(*o.Octree_t)
  ...
  Procedure RecursiveSplitNode(*n.Node_t, n.i)
    If n > 0
      Debug "Recursive Split Node: "+Str(n)
       SplitNode(*n)
  ...
  ; NUM LEAVES
  ;---------------------------------------------------------------------
  Procedure NumLeaves(*o.Octree_t)
    ProcedureReturn 666  
  EndProcedure
  
EndModule

Define *root.Octree::Node_t = Octree::NewNode(16)
Define *o.Octree::Octree_t = Octree::New(*root)
Octree::Delete(*o)
