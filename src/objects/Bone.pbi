...
DeclareModule Bone
  ...
  Structure Bone_t Extends Object3D::Object3D_t
    ;*shape.Shape::Shape_t
    deformdirty.b
    topodirty.b
    radius.f
...
;     eab2.i
  ...
  Interface IBone Extends Object3D::IObject3D
  EndInterface
  
  Declare New(name.s,shape.i)
  Declare Delete(*Me.Bone_t)
  Declare Setup(*Me.Bone_t,*shader.Program::Program_t)
  Declare Update(*Me.Bone_t)
  Declare Clean(*Me.Bone_t)
  Declare Draw(*Me.Bone_t)
  Declare SetFromShape(*Me.Bone_t,shape.i)
  Declare TestClass(*Me.Bone_t)
  Declare OnMessage(id.i, *up)
  Declare SetDirtyState(*Me.Bone_t, state.i)
  Declare SetClean(*Me.Bone_t)
  DataSection 
    PolymeshVT: 
    Data.i @Delete()
    Data.i @Setup()
...
;     *Me\texture = 0
  ...
  ; Set Dirty State
  ;-----------------------------------------------------
  Procedure SetDirtyState(*Me.Bone_t, state)
  ...
  ;-----------------------------------------------------
  Procedure UpdateGLData(*p.Bone_t)
...
;     Next i
  ...
  ; Buil GL Data 
  ;-----------------------------------------------------
  Procedure BuildGLData(*p.Bone_t)
...
;     MessageRequester("Time Difference",StrF(delta1)+Chr(10)+StrF(delta2))
  ...
  ; Buil Edges GL Data 
  ;-----------------------------------------------------
  Procedure BuildGLEdgeData(*p.Bone_t)
...
;     glBufferData(#GL_ELEMENT_ARRAY_BUFFER,size_t,CArray::GetPtr(*geom\a_edgeindices, 0),#GL_STATIC_DRAW)
  ...
  ;-----------------------------------------------------
  Procedure UpdateGLEdgeData(*p.Bone_t)
    Protected *geom.Geometry::PolymeshGeometry_t = *p\geom
    ; Push Position Datas to GPU
    Protected size_t = CArray::GetItemSize(*geom\a_positions) * *geom\nbpoints
    glBufferData(#GL_ARRAY_BUFFER,size_t,CArray::GetPtr(*geom\a_positions, 0),#GL_STATIC_DRAW)
  ...
  ; Setup
  ;----------------------------------------------------
  Procedure Setup(*p.Bone_t,*pgm.Program::Program_t)
...
;     EndIf
  ...
  ; Set From Shape
  ;----------------------------------------------------
  Procedure SetFromShape(*Me.Bone_t,shape.i)
